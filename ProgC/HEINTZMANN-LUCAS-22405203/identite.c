/**
 * @file identite.c
 * @brief Source file for the identite module
 */
 ...
 tIdentite IdentiteLiref(FILE *f)
 {
     ...
     if (fgets(nom, sizeof(nom), f) == NULL) return NULL;
     nom[strcspn(nom, "\n")] = '\0';
 
     if (fgets(prenom, sizeof(prenom), f) == NULL) return NULL;
     prenom[strcspn(prenom, "\n")] = '\0';
 
     if (fgets(ligne, sizeof(ligne), f) == NULL) return NULL;
     sexe = ligne[0];
 
     if (fgets(ligne, sizeof(ligne), f) == NULL) return NULL;
     ligne[strcspn(ligne, "\n")] = '\0';
     if (strlen(ligne) != LG_DATE)
     {
         fprintf(stderr, "Erreur : date invalide '%s'\n", ligne);
         return NULL;
     }
     strncpy(dateNaissance, ligne, LG_DATE);
     dateNaissance[LG_DATE] = '\0';
